...
def warpImages(images, seq, app_h):
    ...
    for i in range(len(seq)):
        h.append((images[seq[i]].shape)[0])
        w.append((images[seq[i]].shape)[1])
    ...
    for i in range(len(images)):
    	rpts.append(cv.perspectiveTransform(pts[i], Ht.dot(app_h[i])))
        warped_images[i] = cv.warpPerspective(images[seq[i]], Ht.dot(app_h[i]), (xmax-xmin, ymax-ymin),flags=cv.INTER_LINEAR+cv.WARP_FILL_OUTLIERS, borderMode=cv.BORDER_CONSTANT, borderValue=0)
        out = "warp" + str(i) + ".jpg" 
        cv.imwrite(out, warped_images[i])
    shape=[ymax-ymin, xmax-xmin]
    return [warped_images, rpts, shape]
...
def bmergeImages2(output):
	h, w = output[2]
	imgs = output[0]
	pts = output[1]
	panorama = np.zeros(shape=[h, w, 3], dtype=np.uint8)
	#print len(imgs)
	for i in range(len(imgs)-1,-1,-1):
		xmin, ymin, xmax, ymax = pts[i][0][0][0], pts[i][0][0][1], pts[i][0][0][0], pts[i][0][0][1]
		for j in range(0,4):
			#print pts[i][j][0]
			if pts[i][j][0][0] < xmin:
				xmin = int(pts[i][j][0][0])
			if pts[i][j][0][0] > xmax:
				xmax = int(pts[i][j][0][0])
			if pts[i][j][0][1] < ymin:
				ymin = int(pts[i][j][0][1])
			if pts[i][j][0][1] > ymax:
				ymax = int(pts[i][j][0][1])
		#print xmin, xmax, ymin, ymax
		x = imgs[i][int(ymin):int(ymax), int(xmin):int(xmax)]
		plt.imshow(x, 'gray'),plt.show()
		panorama[int(ymin):int(ymax), int(xmin):int(xmax)] = x
	return panorama
	
def bmergeImages22(output):
	h, w = output[2]
	imgs = output[0]
	panorama = np.zeros(shape=[h, w, 3], dtype=np.uint8)
	'''img2gray = cv.cvtColor(panorama,cv.COLOR_BGR2GRAY)
	ret, pmask = cv.threshold(img2gray, 1, 255, cv.THRESH_BINARY)
	panorama = cv.bitwise_and(panorama, panorama, mask = pmask)'''
	m_imgs = []
	minival = 255
	for i in range(len(imgs)):
		imi = imgs[i]
		img2gray = cv.cvtColor(imi,cv.COLOR_BGR2GRAY)
		ret, mask = cv.threshold(img2gray, 1, 255, cv.THRESH_BINARY)
		m_imgs.append(mask)
		imiv = imi[:,:,2]
		val = np.mean(imiv[np.where(mask == 255)])
		if minival > val:
			minival = val
	for i in range(len(imgs)):
		imi = imgs[i]
		imiv = imi[:,:,2]
		inten = np.mean(imiv[np.where(m_imgs[i] == 255)])
		imi[...,2] = imi[...,2]*minival/inten
		panorama[np.where(m_imgs[i] == 255)] = imi[np.where(m_imgs[i] == 255)]
...
def createPanorama(folder):
    ...
    for i in range(1,len(seq)):
        ...
        out = "out" + str(i) + ".jpg"
        cv.imwrite(out, warped_img_2)
    cimg = getColorResizedimages(color_images)
    output = warpImages(cimg, seq, app_h)
    panorama = bmergeImages22(output)
    panorama = cv.cvtColor(panorama,cv.COLOR_HSV2BGR)
